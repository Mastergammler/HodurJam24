...
# h for headers
if [[ "$*" == *"-h"* ]]; then
    clang++ -x c++-header $PRECOMPILE_FILE -o $PRECOMPILE_OUTPUT $MSVC_DEFAULTS $DEBUG
fi

PRE_COMPILE_END=$(date +%s%N)
START_TIME=$(date +%s%N)

clang++ $SOURCE_FILE -o $OUTPUT_FILE $INCLUDE_PRECOMPILED $MSVC_DEFAULTS $LIB_PATHS $LINKED_LIBS $OPTIONS
COMPILE_STATUS=$?
